...
const LandingPage = ({ onAuthSuccess }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col p-8 relative transition-colors duration-300 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none opacity-30" style={{ background: 'radial-gradient(ellipse at 20% 20%, var(--color-primary) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, var(--color-accent) 0%, transparent 50%), radial-gradient(ellipse at 50% 50%, var(--color-muted) 0%, transparent 70%)' }} />
      ...
      <main className="flex-1 flex items-center justify-center relative z-10 pb-16">
        <div className="w-full max-w-[1280px] grid grid-cols-1 md:grid-cols-2 gap-20 items-center px-4">
          ...
          <section className="text-foreground space-y-8">
            {/* Logo and title */}
            <div className="mb-10 flex items-center gap-5 animate-fade-in-up opacity-0" style={{ animationDelay: '0.1s' }}>
              ...
              <h1 className="m-0 text-5xl font-bold tracking-tight bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent">
                MitPlan
              </h1>
            </div>

            {/* Subtitle */}
            <h2 className="m-0 text-3xl font-semibold text-muted-foreground leading-tight animate-fade-in-up opacity-0" style={{ animationDelay: '0.2s' }}>
              FFXIV Mitigation Planner
            </h2>

            {/* Description */}
            <p className="text-lg leading-relaxed text-muted-foreground max-w-[540px] animate-fade-in-up opacity-0" style={{ animationDelay: '0.3s' }}>
            ...
            <ul className="list-none p-0 m-0 space-y-4 pt-4">
              {features.map((feature, index) => (
                  <li
                    key={index}
                    className={`flex items-center gap-4 text-base text-foreground animate-fade-in-up opacity-0 stagger-${index + 1}`}
                  >
