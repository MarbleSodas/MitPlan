...
const TimelineSettingsDrawer = ({
  ...
  canEditBossTags = true,
  lockBossTagsMessage = 'Encounter identity is locked for this timeline.',
}) => {
      ...
      <SheetContent>
        ...
        <div className="py-6 space-y-6">
          <div className="space-y-2">
            <Label>Level</Label>
            <PresenceTarget
              target={{ surface: 'timeline', entityType: 'timelineMeta', entityId: 'settings', field: 'level' }}
              ...
              <>
                ...
                <div className="relative mt-3">
                  <div className="flex gap-2">
                    <PresenceTarget
                      target={{ surface: 'timeline', entityType: 'timelineBossTag', entityId: 'tags', field: 'input' }}
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {bossTags.map((tag, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-muted text-foreground rounded-full text-sm"
                        >
                          {boss?.icon && <span>{boss.icon}</span>}
                          {boss ? boss.name : tag}
                        </div>
          ...
          <div className="space-y-2">
            ...
            <PresenceTarget
              target={{ surface: 'timeline', entityType: 'timelineMeta', entityId: 'settings', field: 'description' }}
              className="rounded-md"
              showIndicator={false}
              publishFocus={true}
              focusInteraction="editing"
